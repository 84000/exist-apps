...
declare function glossary:glossary-terms($type as xs:string*, $lang as xs:string) as node() {
    ...
    <glossary
        ...
        lang="{ $lang }">
    {
        for $main-term in distinct-values($glossary:translations//*[@type="glossary"]//tei:gloss[@type = $type]/tei:term[lower-case(@xml:lang) eq lower-case($lang) or (lower-case($lang) eq 'en' and  not(@xml:lang))][not(@type = 'definition')][not(@type = 'alternative')]/text()/normalize-space(.))
            
            let $normalized-term := common:alphanumeric(common:normalized-chars($main-term))
            let $start-letter := substring($normalized-term, 1, 1)
            
            let $items := $glossary:translations//tei:back//tei:gloss[ft:query(tei:term[not(@type = 'definition')], glossary:ft-query($main-term), glossary:ft-options())]
...
declare function glossary:cumulative-glossary() as node() {
    ...
    {
        for $main-term in distinct-values($glossary:translations//*[@type="glossary"]//tei:gloss/tei:term[(lower-case(@xml:lang) = ('eng', 'en') or not(@xml:lang))][not(@type = 'definition')][not(@type = 'alternative')]/text()/normalize-space(.))
...
declare function glossary:glossary-items($term as xs:string) as node() {
    ...
    <glossary
        ...
        <term>{ $term }</term>
        {
            for $item in $glossary:translations//tei:back//tei:gloss[ft:query(tei:term[not(@type = 'definition')], glossary:ft-query($term), glossary:ft-options())]
