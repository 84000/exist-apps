...
declare variable $local:request-content-mode := if(request:exists()) then request:get-parameter('content', 'section') else '';
...
declare function local:catalogue-sections($section-tei as element(tei:TEI), $parent-id as xs:string) as element()* {
    ...
    let $description := $section-tei//tei:front/tei:div[@type eq 'abstract'][node()] ! json-types:normalize-text(.)
    ...
    let $child-works := 
        for $text-bibl in $section:texts//tei:TEI/tei:teiHeader//tei:sourceDesc/tei:bibl[tei:idno[@parent-id eq $section-id]]
            ...
            let $title-migration-id := json-types:title-migration-id($text-bibl/@key, 'eft:toh', $text-bibl/tei:ref, $titles):)
            ...
            return (
                json-types:catalogue-work($text-bibl/@key, $section-id, (:$label, $title-migration-id,:) $text-id, $start-volume/@number, $start-volume/@start-page, $end-volume/@number, $end-volume/@end-page, $text-bibl/tei:location/@count-pages)(:,
                json-types:control-data($section-id, 'work-start-volume', $start-volume/@number ! xs:integer(.)):)
            
            )
    
    return (
        json-types:catalogue-section($section-id, ($parent-id[not(. eq 'LOBBY')], '')[1], $type, $label, $sort-index, $titles, $description),
        json-types:control-data($section-id, 'count-child-sections', count($descendant-sections[self::eft:catalogueSection][@parent_xmlid eq $section-id])),
        json-types:control-data($section-id, 'count-descendant-sections', count($descendant-sections[self::eft:catalogueSection])),
        json-types:control-data($section-id, 'count-child-works', count($child-works)),
        json-types:control-data($section-id, 'count-descendant-works', count($descendant-sections[self::eft:catalogueWork] | $child-works)),
        $descendant-sections,
        $child-works
    )
};
...
declare function local:titles($section-tei as element(tei:TEI)) {
    ...
    let $title-migration-id := json-types:title-migration-id($section-id, $title-type, $title, $titles)
    return
        json-types:title($title-migration-id, $title-language, $section-id, $title-type, json-types:normalize-text($title), $title/@rend ! concat('attestation-', .))
...
element catalogue {
    ...
    attribute url { concat('/rest/catalogue.json?', string-join((concat('api-version=', $json-types:api-version), $local:request-section-id[. gt ''] ! concat('section-id=', $local:request-section-id), $local:request-content-mode[. gt ''] ! concat('content=', $local:request-content-mode)), '&amp;')) },
