...
declare function local:entities() {
    ...
    return (
        ...
        $entity/eft:content[@type eq 'glossary-notes'][descendant::text()[normalize-space()]] ! json-types:annotation($entity/@xml:id, 'contentGlossaryNotes', json-types:normalize-text(.), @timestamp ! xs:dateTime(.), @user),
        ...
        return (
            ...
            return
                if($flag[@type eq 'requires-attention']) then
                    json-types:annotation($instance-id, 'flagRequiresAttention', (), $flag/@timestamp ! xs:dateTime(.), $flag/@user)
                else if($flag[@type eq 'hidden']) then
                    json-types:annotation($instance-id, 'flagHidden', (), $flag/@timestamp ! xs:dateTime(.), $flag/@user)
                else
                    json-types:annotation($instance-id, concat('flag', $flag/@type), (), $flag/@timestamp ! xs:dateTime(.), $flag/@user)
...
declare function local:teams() {
    ...
    return (
        ...
        $team[@rend eq 'hidden'] ! json-types:annotation($team/@xml:id, 'rendHidden', (), (), ())
...
element authorities-annotations {
    ...
    attribute url { concat('/rest/authorities-annotations.json?', string-join((concat('api-version=', $json-types:api-version), $local:request-entity[@xml:id eq $local:request-entity-id] ! concat('id=', $local:request-entity-id), $local:request-text ! concat('text-id=', $local:request-text-id)), '&amp;')) },
