...
declare function local:titles() {
    ...
    let $title-migration-id := types:title-migration-id($source-key, $title-type, $title, $titles)
    
    return (
    
        types:title($title-migration-id, $title-language, $local:text-id, $title-type, helpers:normalize-text($title), $title/@rend ! concat('attestation-', .)),
        ...
        if($title-type eq 'eft:mainTitle' and $chapter-bibl and $section-title[@xml:lang eq $title-language]) then 
            ...
            let $title-migration-id := types:title-migration-id($source-key, $title-type, $title, $titles)
            return
                types:title($title-migration-id, ($title/@xml:lang, 'en')[1], $local:text-id, $title-type, string-join((helpers:normalize-text($section-title), helpers:normalize-text($title)), ', '), ())
...
let $response := 
    element translation {
        ...
        helpers:passages($local:xml-response/eft:response)
