...
<xsl:stylesheet xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template name="generate_excel">
        ...
        <xsl:param name="themes">
            <xsl:copy-of select="$__blank_theme"/>
        </xsl:param>
        <xsl:param name="styles">
            <xsl:copy-of select="$__blank_styles"/>
        </xsl:param>
        <xsl:param name="sheetContents">
            <xsl:copy-of select="$__blank_sheet"/>
        </xsl:param>
        <xsl:param name="vbas"/>
        ...
        <xsl:call-template name="generate_manifest">
            <xsl:with-param name="sheets">
                <xsl:value-of select="count($sheetNames/name)"/>
            </xsl:with-param>
            <xsl:with-param name="drawings">
                <xsl:value-of select="count($images/image/usedIn/sheet)"/>
            </xsl:with-param>
            <xsl:with-param name="media">
                <xsl:copy-of select="$media"/>
            </xsl:with-param>
            <xsl:with-param name="vbas">
                <xsl:copy-of select="$vbas"/>
            </xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:call-template name="generate_media">
            ...
            <xsl:with-param name="printerSettings">
                <xsl:copy-of select="$printerSettings"/>
            </xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:call-template name="generate_sheets">
            ...
            <xsl:with-param name="sheets">
                <xsl:copy-of select="$sheetContents"/>
            </xsl:with-param>
            <xsl:with-param name="vbas">
                <xsl:copy-of select="$vbas"/>
            </xsl:with-param>
