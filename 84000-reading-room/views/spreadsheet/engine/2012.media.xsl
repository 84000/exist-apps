...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://read.84000.co/ns/1.0" version="2.0">
    ...
    <xsl:template name="generate_media">
        ...
        <xsl:for-each-group select="$images/image" group-by="concat(path, '.', ext)">
            <m:entry href="xl/media/{current-grouping-key()}" media-type="text/plain" omit-xml-declaration="yes">
                <xsl:fallback/>
            </m:entry>
        </xsl:for-each-group>
        <xsl:if test="$printerSettings">
            <xsl:for-each select="$printerSettings/setting">
                <m:entry href="xl/printerSettings/{.}" media-type="text/plain" omit-xml-declaration="yes">
                    <xsl:fallback/>
                </m:entry>
            </xsl:for-each>
        </xsl:if>
    ...
    <xsl:template name="gen_media_drawings">
        ...
        <m:entry href="xl/worksheets/_rels/sheet{$sheetNr}.xml.rels">
        ...
        <xsl:if test="$images">
            ...
            <m:entry href="xl/drawings/_rels/drawing{$sheetNr}.xml.rels">
                <Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
                    <xsl:for-each select="$images">
