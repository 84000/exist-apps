...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:m="http://read.84000.co/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="3.0" exclude-result-prefixes="#all">

    <xsl:template name="progress-pie-chart">
        
        <xsl:param name="publications-summary" as="element(m:publications-summary)"/>
        <xsl:param name="replace-text" as="element(m:replace-text)"/>
        <xsl:param name="show-legend" as="xs:boolean" required="yes"/>
        
        <xsl:variable name="chart-id" select="string-join(('eft-progress-chart', $publications-summary/parent::m:translation-summary/@section-id ! string(.)), '-')"/>
        <xsl:variable name="percent-published" select="xs:integer((sum($publications-summary/m:pages/@published ! xs:integer(.)) div sum($publications-summary/m:pages/@total ! xs:integer(.))) * 100)" as="xs:integer"/>
        <xsl:variable name="percent-translated" select="xs:integer((sum($publications-summary/m:pages/@translated ! xs:integer(.)) div sum($publications-summary/m:pages/@total ! xs:integer(.))) * 100)" as="xs:integer"/>
        <xsl:variable name="percent-in-translation" select="xs:integer((sum($publications-summary/m:pages/@in-translation ! xs:integer(.)) div sum($publications-summary/m:pages/@total ! xs:integer(.))) * 100)" as="xs:integer"/>
