...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:common="http://read.84000.co/common" xmlns:util="http://exist-db.org/xquery/util" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://read.84000.co/ns/1.0" xmlns:xhtml="http://www.w3.org/1999/xhtml" version="3.0" exclude-result-prefixes="#all">
    ...
    <xsl:variable name="template" select="/m:response/m:request/@template" as="xs:string?"/>
    <xsl:variable name="url-folder" select="if($template eq 'embedded') then 'glossary-embedded' else 'glossary'"/>
    <xsl:variable name="url-resource" select="if($active-tab eq 'downloads') then 'downloads' else 'search'"/>
    ...
    <xsl:variable name="page-title" as="xs:string">
        <xsl:choose>
            <xsl:when test="$selected-letter">
                <xsl:value-of select="concat(string-join(($selected-term-lang/text(), $selected-type/m:label[@type eq 'plural']/text(), $selected-letter ! concat('filtered by &#34;', ., '&#34;')), ', '), ' in the Glossary')"/>
            </xsl:when>
            <xsl:when test="$search-text gt ''">
                <xsl:value-of select="concat(string-join(($selected-term-lang/text(), $selected-type/m:label[@type eq 'plural']/text(), $search-text ! concat('searching for &#34;', ., '&#34;')), ', '), ' in the Glossary')"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:variable name="page-url" select="concat('/', $url-folder, '/', $url-resource, '.html?') || string-join(($selected-term-lang ! concat('term-lang=', @id), $selected-type ! concat('term-type[]=', @id), $selected-letter ! concat('letter=', @index), concat('search=', $search-text), $selected-sort ! concat('sort=', .), $flagged ! concat('flagged=', .)), '&amp;')" as="xs:string"/>
    ...
    <xsl:template match="/m:response">
        ...
        <xsl:variable name="content">
            ...
            <!-- Title band -->
            <xsl:if test="not($template eq 'embedded')">
                        ...
                        <div class="center-vertical-sm full-width">
                            ...
                            <nav role="navigation" aria-label="Breadcrumbs">
                                <ul class="breadcrumb">
                                    ...
                                    <li>
                                        <a>
                                            <xsl:attribute name="href" select="common:internal-href(concat('/', $url-folder, '/search.html'), (), (), /m:response/@lang)"/>
                    ...
                    <div class="tab-content">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                        ...
                                        <div>
                                            ...
                                            <div class="tabs-container-center">
                                                <ul class="nav nav-pills" role="tablist" id="nav-letters">
                                                    ...
                                                    <xsl:variable name="internal-link-attrs" select="(concat('term-lang=', $selected-term-lang/@id), $selected-type ! concat('term-type[]=', ./@id), $selected-sort ! concat('sort=', .), m:view-mode-parameter())"/>
                                                    ...
                                                    <xsl:for-each select="$alphabet/m:letter">
                                                        ...
                                                        <li role="presentation" class="letter">
                                                            ...
                                                            <a>
                                                                ...
                                                                <xsl:attribute name="href" select="common:internal-href(concat('/', $url-folder, '/search.html?letter=', @index), $internal-link-attrs, (), $root/m:response/@lang)"/>
                                ...
                                <!-- Search form -->
                                <form action="{ concat('/', $url-folder, '/search.html') }" method="get" role="search" class="form-inline" data-loading="Searching...">
                                        ...
                                        <div>
                                            <select name="sort" class="form-control">
                                                ...
                                                <option value="frequency">
                                                    ...
                                                    <xsl:value-of select="'Sort by frequency'"/>
                                                </option>
                                            </select>
                                        </div>
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="$entities-list">
                                                        ...
                                                        <a class="entity-title block-link opener-link log-click">
                                                            ...
                                                            <div class="search-matches top-vertical-sm full-width">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <ul class="list-inline inline-dots term-list primary-terms">
                                        ...
                                        <!-- Pagination -->
                                        <xsl:sequence select="common:pagination(m:request/@first-record, m:request/@records-per-page, m:entities/@count-entities, common:internal-href($page-url, m:view-mode-parameter(), (), /m:response/@lang))"/>
                                                ...
                                                <xsl:when test="$search-text gt ''">
                                                    ...
                                                    <ul class="list-inline inline-dots">
                                                        <xsl:for-each select="/m:response/m:request/m:term-langs/m:lang[not(@selected)]">
                                                            <li>
                                                                <a class="underline">
                                                                    <xsl:attribute name="href" select="common:internal-href(concat('/', $url-folder, '/search.html?search=', $search-text), (concat('term-lang=', @id), $selected-type ! concat('term-type[]=', @id), $selected-sort ! concat('sort=', .), m:view-mode-parameter()), (), $root/m:response/@lang)"/>
                                                                    <xsl:value-of select="text()"/>
                                                                </a>
                                                            </li>
                                                        </xsl:for-each>
                                                        <xsl:if test="/m:response/m:request/m:entity-types/m:type[@glossary-type][not(@selected)]">
                                                            <li>
                                                                <a class="underline">
                                                                    <xsl:attribute name="href" select="common:internal-href(concat('/', $url-folder, '/search.html?search=', $search-text), (concat('term-lang=', $selected-term-lang/@id), $selected-sort ! concat('sort=', .), m:view-mode-parameter()), (), $root/m:response/@lang)"/>
        ...
        <xsl:choose>
            <xsl:when test="$template eq 'embedded'">
                <xsl:call-template name="widget-page">
                    <xsl:with-param name="page-url" select="$page-url"/>
                    <xsl:with-param name="page-class" select="'embedded'"/>
