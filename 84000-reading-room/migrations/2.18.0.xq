...
declare function local:update-versions() {
    ...
    let $gloss-revisions := 
        ...
        where 
            $gloss/tei:term[@xml:lang eq 'Sa-Ltn'][matches(text(), '^[ABCDEFGHIJKLMNOPQRSTUVWXYZĀḌÉḤĪḶḸṂṆÑṄṚṜṢŚṬŪṀ]')]
            or $gloss/tei:term[@type = ('sourceAttested','semanticReconstruction','transliterationReconstruction')]
        return
            $gloss
    
    where $gloss-revisions and $text-id = ('UT22084-048-001')
    return (
        (:$text-id,:)
