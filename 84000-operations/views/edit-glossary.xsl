...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:output="http://www.w3.org/2010/xslt-xquery-serialization" xmlns:scheduler="http://exist-db.org/xquery/scheduler" xmlns:common="http://read.84000.co/common" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:m="http://read.84000.co/ns/1.0" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:ops="http://operations.84000.co" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="3.0" exclude-result-prefixes="#all">
            ...
            <xsl:call-template name="operations-page">
                ...
                <xsl:with-param name="tab-content">
                    ...
                    <header>
                        ...
                        <a target="_self" class="small underline">
                            <xsl:choose>
                                <xsl:when test="$request-resource-type eq 'knowledgebase'">
                    ...
                    <xsl:if test="$request-filter eq 'blank-form'">
                        ...
                        <div id="selected-entry" class="data-container replace">
                            
                            <!-- Form: add new entry -->
                            <xsl:call-template name="form">
                                
                                <xsl:with-param name="form-id" select="'glossary-form-blank'"/>
                                    ...
                                    <xsl:if test="$request-entity">
                                        ...
                                        <div class="checkbox">
                                            <label>
                                                ...
                                                <xsl:value-of select="'use shared entity'"/>
                                            </label>
                                        </div>
                                    ...
                                    <div class="list-group accordion accordion-background" role="tablist" aria-multiselectable="false">
                                        ...
                                        <xsl:if test="$request-filter = ('check-entities', 'check-terms', 'check-people', 'check-places', 'check-texts', 'check-all', 'missing-entities', 'requires-attention', 'entity-definition', 'shared-entities', 'exclusive-entities')">
                                            ...
                                            <xsl:if test="$loop-glossary-entity">
                                                ...
                                                <xsl:variable name="related-entry-texts" select="/m:response/m:entities/m:related/m:text[m:entry/@id = $loop-glossary-entity/m:instance/@id]"/>
                                                ...
                                                <xsl:call-template name="expand-item">
                                                    ...
                                                    <xsl:with-param name="title">
                                                        ...
                                                        <xsl:if test="not(/m:response/m:entity-types/m:type[@id = $loop-glossary-entity/m:type/@type][@glossary-type eq $loop-glossary/@type])">
                                                            <xsl:value-of select="' '"/>
                                                            <span class="label label-warning">
    ...
    <xsl:template name="link-href">
        ...
        <xsl:value-of select="concat('/edit-glossary.html?', string-join($parameters, '&amp;'),'#selected-entry')"/>
    ...
    <xsl:template name="glossary-form">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <input type="hidden" name="ajax-target" value="selected-item"/>
            </xsl:otherwise>
        </xsl:choose>
            ...
            <xsl:variable name="entry-type" as="xs:string?">
                <xsl:choose>
                    ...
                    <xsl:when test="$entity[m:type]">
                        <xsl:value-of select="(/m:response/m:entity-types/m:type[@id = $entity/m:type/@type])[last()]/@glossary-type"/>
                    </xsl:when>
                    <xsl:otherwise>
        ...
        <div class="form-group">
            <div class="col-sm-12">
                ...
                <xsl:if test="/m:response/m:text[not(@locked-by-user gt '')] and $entry[@id]">
                            ...
                            <xsl:with-param name="target-entity-label">
                                ...
                                <xsl:variable name="entity-instance-entry-definition" as="xs:string?">
                                    <xsl:if test="count($entity/m:instance) eq 1">
                                        <xsl:value-of select="string-join(/m:response/m:entities/m:related/m:text/m:entry[@id = $entity/m:instance/@id]/m:definition/tei:p[descendant::text()[normalize-space()]] ! string-join(descendant::text()), ' ')"/>
                                    </xsl:if>
                                </xsl:variable>
