...
return
    ...
    if(not(doc(concat('/db/system/config',$common:tei-path, '/collection.xconf'))/ex:collection/ex:triggers)) then 
        ...
        let $entities-with-source-with-stub := $entities:entities//m:entity[m:source[matches(@key, concat('^', functx:escape-for-regex($import-key), '#'))]][m:instance[@type eq 'knowledgebase-article']]
        let $entities-with-source-without-stub := $entities:entities//m:entity[m:source[matches(@key, concat('^', functx:escape-for-regex($import-key), '#'))]][not(m:instance[@type eq 'knowledgebase-article'])]
        
        return (
        
            element progress {
                attribute count-source-entities-with-stubs { count($entities-with-source-with-stub) },
                attribute count-source-entities-without-stubs { count($entities-with-source-without-stub) }
            },
        
            for $entity in subsequence($entities-with-source-without-stub,1,10)
            ...
            let $authour-attributions := $local:tengyur-tei/tei:teiHeader//tei:sourceDesc/tei:bibl/tei:author[not(@role)][@ref eq concat('eft:', $entity/@xml:id)]
